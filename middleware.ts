...
export async function middleware(request: NextRequest) {
  ...
  const isProtected =
    ...
    pathname.match(/^\/jobs\/.+/) ||                 // /jobs/{id} but NOT /jobs
...
export const config = {
  matcher: ['/mein-pex/:path*', '/jobs/:path+', '/upload/:path*', '/api/upload/:path*', '/api/conversations/:path*', '/cv-check/result/:path*', '/cv-check/optimize/:path*', '/api/cv-check/results/:path*', '/api/cv-check/download/:path*', '/api/cv-check/optimize/:path*', '/api/cv-check/create-checkout/:path*'],
}
